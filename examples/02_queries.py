...
def main():
    ...
    print("contains electronics:", [p.name for p in Product.query().filter(F("tags").contains("electronics")).all()])
    print("isin computers:", [p.name for p in Product.query().filter(F("tags").isin(["computers"])) .all()])
    print("any qty>3:", [p.name for p in Product.query().filter(F(["items"]).any()["qty"] > 3).all()])
